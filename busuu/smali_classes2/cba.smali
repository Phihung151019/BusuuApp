.class public final Lcba;
.super Lrwe;
.source "SourceFile"


# static fields
.field public static final o:[B

.field public static final p:[B


# instance fields
.field public n:Z


# direct methods
.method static constructor <clinit>()V
    .locals 2

    const/16 v0, 0x8

    new-array v1, v0, [B

    fill-array-data v1, :array_0

    sput-object v1, Lcba;->o:[B

    new-array v0, v0, [B

    fill-array-data v0, :array_1

    sput-object v0, Lcba;->p:[B

    return-void

    nop

    :array_0
    .array-data 1
        0x4ft
        0x70t
        0x75t
        0x73t
        0x48t
        0x65t
        0x61t
        0x64t
    .end array-data

    :array_1
    .array-data 1
        0x4ft
        0x70t
        0x75t
        0x73t
        0x54t
        0x61t
        0x67t
        0x73t
    .end array-data
.end method

.method public constructor <init>()V
    .locals 0

    invoke-direct {p0}, Lrwe;-><init>()V

    return-void
.end method

.method public static n(Lgoa;[B)Z
    .locals 4

    invoke-virtual {p0}, Lgoa;->a()I

    move-result v0

    array-length v1, p1

    const/4 v2, 0x0

    if-ge v0, v1, :cond_0

    return v2

    :cond_0
    invoke-virtual {p0}, Lgoa;->f()I

    move-result v0

    array-length v1, p1

    new-array v1, v1, [B

    array-length v3, p1

    invoke-virtual {p0, v1, v2, v3}, Lgoa;->l([BII)V

    invoke-virtual {p0, v0}, Lgoa;->U(I)V

    invoke-static {v1, p1}, Ljava/util/Arrays;->equals([B[B)Z

    move-result p0

    return p0
.end method

.method public static o(Lgoa;)Z
    .locals 1

    sget-object v0, Lcba;->o:[B

    invoke-static {p0, v0}, Lcba;->n(Lgoa;[B)Z

    move-result p0

    return p0
.end method


# virtual methods
.method public f(Lgoa;)J
    .locals 2

    invoke-virtual {p1}, Lgoa;->e()[B

    move-result-object p1

    invoke-static {p1}, Ldba;->e([B)J

    move-result-wide v0

    invoke-virtual {p0, v0, v1}, Lrwe;->c(J)J

    move-result-wide v0

    return-wide v0
.end method

.method public i(Lgoa;JLrwe$b;)Z
    .locals 2
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Landroidx/media3/common/ParserException;
        }
    .end annotation

    sget-object p2, Lcba;->o:[B

    invoke-static {p1, p2}, Lcba;->n(Lgoa;[B)Z

    move-result p2

    const/4 p3, 0x1

    if-eqz p2, :cond_1

    invoke-virtual {p1}, Lgoa;->e()[B

    move-result-object p2

    invoke-virtual {p1}, Lgoa;->g()I

    move-result p1

    invoke-static {p2, p1}, Ljava/util/Arrays;->copyOf([BI)[B

    move-result-object p1

    invoke-static {p1}, Ldba;->c([B)I

    move-result p2

    invoke-static {p1}, Ldba;->a([B)Ljava/util/List;

    move-result-object p1

    iget-object v0, p4, Lrwe$b;->a:Lck5;

    if-eqz v0, :cond_0

    return p3

    :cond_0
    new-instance v0, Lck5$b;

    invoke-direct {v0}, Lck5$b;-><init>()V

    const-string v1, "audio/opus"

    invoke-virtual {v0, v1}, Lck5$b;->o0(Ljava/lang/String;)Lck5$b;

    move-result-object v0

    invoke-virtual {v0, p2}, Lck5$b;->N(I)Lck5$b;

    move-result-object p2

    const v0, 0xbb80

    invoke-virtual {p2, v0}, Lck5$b;->p0(I)Lck5$b;

    move-result-object p2

    invoke-virtual {p2, p1}, Lck5$b;->b0(Ljava/util/List;)Lck5$b;

    move-result-object p1

    invoke-virtual {p1}, Lck5$b;->K()Lck5;

    move-result-object p1

    iput-object p1, p4, Lrwe$b;->a:Lck5;

    return p3

    :cond_1
    sget-object p2, Lcba;->p:[B

    invoke-static {p1, p2}, Lcba;->n(Lgoa;[B)Z

    move-result v0

    const/4 v1, 0x0

    if-eqz v0, :cond_4

    iget-object v0, p4, Lrwe$b;->a:Lck5;

    invoke-static {v0}, Lva0;->i(Ljava/lang/Object;)Ljava/lang/Object;

    iget-boolean v0, p0, Lcba;->n:Z

    if-eqz v0, :cond_2

    return p3

    :cond_2
    iput-boolean p3, p0, Lcba;->n:Z

    array-length p2, p2

    invoke-virtual {p1, p2}, Lgoa;->V(I)V

    invoke-static {p1, v1, v1}, Ltkh;->k(Lgoa;ZZ)Ltkh$a;

    move-result-object p1

    iget-object p1, p1, Ltkh$a;->b:[Ljava/lang/String;

    invoke-static {p1}, Ln37;->R([Ljava/lang/Object;)Ln37;

    move-result-object p1

    invoke-static {p1}, Ltkh;->d(Ljava/util/List;)Ly79;

    move-result-object p1

    if-nez p1, :cond_3

    return p3

    :cond_3
    iget-object p2, p4, Lrwe$b;->a:Lck5;

    invoke-virtual {p2}, Lck5;->a()Lck5$b;

    move-result-object p2

    iget-object v0, p4, Lrwe$b;->a:Lck5;

    iget-object v0, v0, Lck5;->k:Ly79;

    invoke-virtual {p1, v0}, Ly79;->b(Ly79;)Ly79;

    move-result-object p1

    invoke-virtual {p2, p1}, Lck5$b;->h0(Ly79;)Lck5$b;

    move-result-object p1

    invoke-virtual {p1}, Lck5$b;->K()Lck5;

    move-result-object p1

    iput-object p1, p4, Lrwe$b;->a:Lck5;

    return p3

    :cond_4
    iget-object p1, p4, Lrwe$b;->a:Lck5;

    invoke-static {p1}, Lva0;->i(Ljava/lang/Object;)Ljava/lang/Object;

    return v1
.end method

.method public l(Z)V
    .locals 0

    invoke-super {p0, p1}, Lrwe;->l(Z)V

    if-eqz p1, :cond_0

    const/4 p1, 0x0

    iput-boolean p1, p0, Lcba;->n:Z

    :cond_0
    return-void
.end method
