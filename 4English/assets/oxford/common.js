...
(function($){
	var AutoCompleter = function(element, settings){
		...
		elem.unbind("change.autocomplete");
		elem.bind("change.autocomplete", function(e){
			obj.delayedAutoComplete(e);
		});
		elem.unbind("input.autocomplete");
		elem.bind("input.autocomplete", function(e){
...
(function($){
    var CustomSelector = function(element){
        ...
        obj.refresh = function() {
            ...
            obj.csm.find("a[data-value='" + val + "']").append("<i class='icon-tick oup_icons'></i>");
...
var browser = {firefox:false},
    ...
    dictSel = $("#dictionary-selector"),

    doc = document, 

    oSearch = doc.getElementById('q'),

    jqLetters = $("#keyboard_letters"), 

    letters = doc.getElementById("keyboard_letters");
...
function initInputVal() {
        ...
        var form = doc.getElementById('search-form'),
            ...
            val = doc.getElementById('dictionary-selector').value;
...
function LowerKeyboard() {
    ...
    var keyboardDataLower = ["à", "á", "â", "ä", "ã", "ç", "è", "é", "ê", "ë", "ì", "í", "î", "ï", "ñ", "ò", "ó", "ô", "ö", "õ", "ù", "ú", "û", "ü", "æ", "œ", "ß"],
        ...
        keyboardDataUpper = ["À", "Á", "Â", "Ä", "Ã", "Ç", "È", "É", "Ê", "Ë" ,"Ì", "Í", "Î", "Ï", "Ñ", "Ò", "Ó", "Ô", "Ö", "Õ", "Ù", "Ú", "Û", "Ü", "Æ", "Œ", "SS"];
...
$(doc).ready(function () {
    ...
    if (navigator.userAgent.indexOf("Firefox") > -1) browser.firefox=true;
    ...
    $(".premium-box-toggle").click(function(){
        var element = $(this).parent().parent();
        if(element.hasClass("data-fold")) {
            element.removeClass("data-fold");
            $.cookie("otherLinksFold", null, {expires:7, path: '/'});
        } else {
            element.addClass("data-fold");
            $.cookie("otherLinksFold", true, {expires:7, path: '/'});
        };
    });
...
function initLangSelector(id) {   
    ...
    var selectedLangCode = $(id+' option:selected').attr("name"); 

    if (userVersion != null && userVersion != selectedLangCode) {

        var redirUrl = $(id+" option[name='" + userVersion + "']").val();
